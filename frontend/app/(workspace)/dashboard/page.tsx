...
export default function Dashboard() {
  ...
  const today = tasks.filter(
    (task) =>
      task.status !== "DONE" && task.dueDate && task.dueDate <= todayKey,
  );
  return (
    <>
      ...
      <section className="space-switcher" aria-label="Spaces">
        {spaces.map(({ slug, name, icon: Icon }) => (
          <Link key={slug} href={`/spaces/${slug}`} className={slug}>
            <span>
              <Icon />
            </span>
            <div>
              ...
              <small>
                {
                  (projects.data || []).filter(
                    (project) => project.space === slug.toUpperCase(),
                  ).length
                }{" "}
                projects
              </small>
      ...
      <section className="home-section">
        <div className="product-section-heading">
          ...
          {(projects.data || []).length > 0 && (
            <Link className="text-link" href="/spaces/school">
              View spaces
            </Link>
          )}
        </div>
        {(projects.data || []).length ? (
          ...
          <div className="home-empty">
            ...
            <div>
              ...
              <p>
                Create a project in School, Work, or Personal to keep its tasks
            ...
            <ul className="mini-task-list">
              {today.slice(0, 4).map((task) => (
                <li key={task.id}>
                  ...
                  <time>{task.dueDate === todayKey ? "Today" : "Overdue"}</time>
                </li>
              ))}
            </ul>
          ) : (
            <p className="panel-empty">
              Nothing due today. A little breathing room.
            </p>
          )}
        </section>
        <section className="home-panel">
          <div className="panel-title">
            ...
            <div>
              ...
              <p>Proposals waiting for your review.</p>
            </div>
            <Link href="/inbox">
          ...
          {approvals.data?.length ? (
            <div className="attention-count">
              ...
              <span>
                {approvals.data.length === 1 ? "item is" : "items are"} ready to
                review
              </span>
            </div>
          ) : (

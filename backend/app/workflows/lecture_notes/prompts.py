...
Prefer substantial study-note sections over many tiny slide-heading fragments.
Each section should synthesize related ideas into explanatory paragraphs.
Do not simply repeat slide titles.
Use the overview to explain the lecture's main thread in a concise paragraph.
Use key_concepts for the core concepts a student must understand.
Capitalize field content naturally.
Use takeaways for only the five most important high-level conclusions.
Avoid duplicate takeaways and minor details.
Create review_questions answerable from the material and focused on understanding, not trivia.
Create quiz_questions as a mini lecture quiz with concise answers hidden in Notion toggles.
Each quiz answer must be grounded in the supplied material.
Distinguish definitions from examples. Extract formulas only when present; otherwise use [].
Retain valid section-level source_refs and page numbers when supplied.
Do not claim sentence-level attribution.
